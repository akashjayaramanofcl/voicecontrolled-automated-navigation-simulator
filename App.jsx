...
import { Mic, Play, RotateCcw, Timer, Target, Navigation, Wifi, WifiOff } from 'lucide-react';
...
function App() {
  ...
  const processVoiceCommand = (command) => {
    ...
    if (destination && scenario) {
      if (SCENARIOS[scenario].locations.includes(destination)) {
        ...
        showMessage(`🎯 Navigating to ${destination.toUpperCase()}...`, 'success');
      } else {
        const availableList = SCENARIOS[scenario].locations.join(', ');
        showMessage(`❌ "${destination}" not available. Try: ${availableList}`, 'error');
      }
    } else if (!destination) {
      const availableList = SCENARIOS[scenario].locations.join(', ');
      showMessage(`❓ Location not recognized. Available: ${availableList}`, 'warning');
    }
  };
  ...
  const calculatePath = async () => {
    ...
    try {
      if (backendConnected) {
        ...
        const response = await axios.post(`${BACKEND_URL}/pathfind`, {
          grid: grid,
          start: robotPos,
          end: targetPos
        }, { timeout: 5000 });
        ...
        if (response.data.success && response.data.path) {
          ...
          showMessage(`✅ Path calculated! ${response.data.path.length} steps (${response.data.computation_time_ms.toFixed(1)}ms)`, 'success');
        } else {
          console.error('No path found by backend');
          showMessage('❌ No path available - destination unreachable!', 'error');
          setPath([]);
        }
      } else {
        ...
        console.log('Client-side result:', newPath ? newPath.length + ' steps' : 'No path');
        
        if (newPath && newPath.length > 0) {
          setPath(newPath);
          setComputationTime(compTime);
          showMessage(`✅ Path calculated! ${newPath.length} steps (Client: ${compTime}ms)`, 'success');
        } else {
      ...
      if (newPath && newPath.length > 0) {
        setPath(newPath);
        setComputationTime(compTime);
        showMessage(`⚠️ Using fallback pathfinding: ${newPath.length} steps (${compTime}ms)`, 'warning');
      } else {
  ...
  const handleGridClick = (row, col) => {
    if (isMoving) return;
    if (row === robotPos.row && col === robotPos.col) return;
    if (targetPos && row === targetPos.row && col === targetPos.col) return;
        ...
        <div className="menu-container">
          <div className="menu-header">
            <h1 className="menu-title">
              <img src="/logo.png" alt="VoiceRover Logo" className="title-logo" />
              VoiceRover
            </h1>
          ...
          <div className="instructions-card">
            ...
            <ul className="instructions-list">
              <li>🎤 Press the microphone button and say: "Go to [location]"</li>
              <li>🗺️ Click grid cells to place obstacles dynamically</li>
              <li>🤖 Watch the robot automatically recalculate its path in real-time</li>
              <li>⏱️ Complete as many missions as possible before time runs out!</li>
            </ul>
          </div>
        </div>
      </div>
        ...
        <div className="header-content">
          <div className="header-left">
            ...
            <div className="scenario-info">
              <span className="scenario-icon-small">{currentScenario.icon}</span>
              <div>
            ...
            {grid.map((row, i) => (
              <div key={i} className="grid-row">
                {row.map((cell, j) => {
                  ...
                  const isTarget = targetPos && targetPos.row === i && targetPos.col === j;
                ...
                {SCENARIOS[scenario].locations.map(loc => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      showMessage(`🎯 Target set to ${loc.toUpperCase()}`, 'success');
                    }}
                    disabled={isMoving}
                    style={{
